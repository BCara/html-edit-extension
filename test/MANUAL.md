...
| Try this | Expected |
| --- | --- |
| Hover a paragraph | Two small buttons appear below it: an indigo **+** and an amber **speech bubble** |
| Click the speech bubble | A card opens in a margin on the right, focused. The page shifts left to make room. The paragraph is shaded amber with a bar down its left side |
| Type a note, click elsewhere | The card keeps it. The status bar counts it as a change |
| Comment on a second, lower section | Its card lines up beside *that* section. If the two would overlap, the lower one slides down |
| Click the **×** on a card | The comment goes, the shading goes, the margin closes if it was the last one |
| **Ctrl/Cmd + Z** | Brings a deleted comment back; again undoes a wording change |
| Add a comment, type nothing, **Save** | It is not written. The status bar says `1 empty not saved` |
| Click **Done** | The margin closes and the page returns to its normal width |

Then save and look at the file:

```bash
grep -n 'comment:' ~/Downloads/simple.html
```

✅ **Expect:** `<!-- comment: your note -->` on its own line, immediately before
the section it belongs to, indented to match.

**Then the part that matters for sharing** — open the saved file in Chrome
...
✅ **Expect:** *no* comment cards at all. That file is full of comments — a
conditional comment, a "Generated by hand" note — and none of them are Quick
Edit's, so none are shown and none are touched.
